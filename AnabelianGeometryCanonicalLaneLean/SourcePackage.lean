/-!
# Source package model for `anabelian-geometry-canonical-lane`
...
def sourceCheckoutHead : String := "986c149c04e06c980f8e0912de4ef8446f58d309"
def sourceCheckoutClean : Bool := true

def sourceFiles : List SourceFile := [
  { path := "scripts/ana_closure_guard.py", sha256 := "e3a03d62846810f724b5bd6b0d081f6955c294dae4521d1fd00022d2e9f6ccef", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  { file := "scripts/ana_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 20, isAsync := false },
  { file := "scripts/ana_closure_guard.py", name := "_resolve", args := ["p"], returns := "Path", doc := "", line := 24, isAsync := false },
  { file := "scripts/ana_closure_guard.py", name := "_bootstrap", args := ["path"], returns := "None", doc := "", line := 29, isAsync := false },
  { file := "scripts/ana_closure_guard.py", name := "_load_registry", args := ["path"], returns := "", doc := "", line := 35, isAsync := false },
  { file := "scripts/ana_closure_guard.py", name := "_load_sigma", args := ["path"], returns := "", doc := "", line := 44, isAsync := false },
  { file := "scripts/ana_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "", doc := "", line := 61, isAsync := false },
  { file := "scripts/ana_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict"], returns := "", doc := "", line := 71, isAsync := false },
  { file := "scripts/ana_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "", doc := "", line := 97, isAsync := false },
  { file := "scripts/ana_closure_guard.py", name := "main", args := [], returns := "", doc := "", line := 104, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  { file := "scripts/ana_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 10 },
  { file := "scripts/ana_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 11 },
  { file := "scripts/ana_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_fundamental\", \"sigma_reconstruction\", \"kappa_compact\", \"rho_rigidity\", \"section_transfer\"]", line := 12 },
  { file := "scripts/ana_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 13 },
  { file := "scripts/ana_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 14 },
  { file := "scripts/ana_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 15 },
  { file := "scripts/ana_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 16 },
  { file := "scripts/ana_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 17 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_fundamental", status := "derived_numeric", formula := "c_fund_raw * fund_density_raw - e_fund_raw", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["c_fund_raw", "e_fund_raw", "fund_density_raw"] },
  { group := "constants", key := "sigma_reconstruction", status := "derived_numeric", formula := "reconstruction_floor_raw - galois_loss_raw - restart_loss_raw", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["galois_loss_raw", "reconstruction_floor_raw", "restart_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "section_transfer", status := "derived_numeric", formula := "c_section_raw * transfer_gain_raw - e_section_raw", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_section_raw", "e_section_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Appendix E.6" },
  { key := "kappa_compact", value := "0.8038585209003215", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "kappa_fundamental", value := "1.094116", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_fundamental", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "rho_rigidity", value := "1.078", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "section_transfer", value := "1.0305400000000002", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#section_transfer", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "sigma_reconstruction", value := "1.075", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_reconstruction", sourceSection := "paper/ANABELIAN_GEOMETRY_PREPRINT.md Section 5.2 / Appendix B" }
]
